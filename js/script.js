...
let model = 'llama-3.2-90b-text-preview';
let GROQ_ENDPOINT = "https://api.groq.com/openai/v1/chat/completions";

let MY_GROQ_API_KEY = localStorage.getItem("MY_GROQ_API_KEY");
...
function groqChat() {
    ...
    fetch(GROQ_ENDPOINT, requestOptions)
        ...
        .finally(() => {
            ...
            enableChat();
            if (invalid_key) {
                localStorage.setItem('MY_GROQ_API_KEY', ''); // clean api key
                setApiKeyDialog();
            }
            hljs.highlightAll();
            enableCopyForCode();
...
function setApiKey() {
    ...
    if (set_api_key) {
        ...
        if (api_key.length > 10) {
            MY_GROQ_API_KEY = api_key;
            localStorage.setItem("MY_GROQ_API_KEY", api_key);
            closeDialogs();
...
function setApiKeyDialog() {
    let cnt =
        `<div>Enter your Groq API key!</div>
         ...
         <a target="_blank" href="https://console.groq.com/keys">https://console.groq.com/keys</a>
         </div>`;
